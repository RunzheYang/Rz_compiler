...
public class TypeAnalyser {
    ...
    public Type getTypeofType(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        if (ctx instanceof RzParser.CLASSTYPEContext) {
            ...
            String filePos = "line " +  ((RzParser.CLASSTYPEContext) ctx).start.getLine()
                    + ":" + ((RzParser.CLASSTYPEContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: undefined class type '" + ctx.getChild(0).getText() + "'");
        }
        ...
        String filePos = "line " + ((RzParser.TypeContext)ctx).start.getLine()
                + ":" + ((RzParser.TypeContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid type '" + ctx.getChild(0).getText() + "'");
    }
    ...
    public Type getTypeofBaseType(ParseTree ctx) throws SemanticException {
        ...
        String filePos = "line " +  ((RzParser.CLASSTYPEContext) ctx).start.getLine()
                + ":" + ((RzParser.CLASSTYPEContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid base type '" + ctx.getText() + "'");
    }
    ...
    public Type getTypeofExpression(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        if (ctx instanceof RzParser.MULTIContext) {
            ...
            String filePos = "line " +  ((RzParser.MULTIContext) ctx).start.getLine()
                    + ":" + ((RzParser.MULTIContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: Invalid multiple expression '" + ctx.getText() + "'");
        }
        ...
        if (ctx instanceof RzParser.ADDITIVEContext) {
            ...
            String filePos = "line " +  ((RzParser.ADDITIVEContext) ctx).start.getLine()
                    + ":" + ((RzParser.ADDITIVEContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: Invalid additive expression '" + ctx.getText() + "'");
        }
        ...
        if (ctx instanceof RzParser.SHIFTContext) {
            ...
            String filePos = "line " +  ((RzParser.SHIFTContext) ctx).start.getLine()
                    + ":" + ((RzParser.SHIFTContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: Invalid shift expression '" + ctx.getText() + "'");
        }
        ...
        if (ctx instanceof RzParser.RELATIONContext) {
            ...
            String filePos = "line " +  ((RzParser.RELATIONContext) ctx).start.getLine()
                    + ":" + ((RzParser.RELATIONContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: Invalid comparing expression '" + ctx.getText() + "'");
        }
        ...
        if (ctx instanceof RzParser.EQUALITYContext) {
            ...
            String filePos = "line " +  ((RzParser.EQUALITYContext) ctx).start.getLine()
        ...
        if (ctx instanceof RzParser.ANDContext
            ...
            throw new SemanticException(filePos + "Semantic Error: Invalid logic or bit operation '" + ctx.getText() + "'");
        }
        String filePos = "line " +  ((RzParser.ExpressionContext) ctx).start.getLine()
                + ":" + ((RzParser.ExpressionContext)ctx).start.getCharPositionInLine() + " -> ";
    ...
    public Type getTypeofCreationExpr(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        String filePos = "line " +  ((RzParser.Creation_exprContext) ctx).start.getLine()
                + ":" + ((RzParser.Creation_exprContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid creation expression '" + ctx.getText() + "'");
    }
            ...
            if (ctx.getChild(1) instanceof RzParser.Unary_exprContext) {
                ...
                if (ctx.getChild(0).getText().equals("++") || ctx.getChild(0).getText().equals("--")) {
                    if (!(getIdentofUnaryExpr(ctx.getChild(1), symt) instanceof Variable)) {
                        String filePos = "line " +  ((RzParser.Unary_exprContext) ctx).start.getLine()
                                + ":" + ((RzParser.Unary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                        throw new SemanticException(filePos + "Semantic Error: '++' or '--' cannot be apply to the '" + ctx.getChild(1).getText() + "'");
                    }
                }
                ...
                String filePos = "line " +  ((RzParser.Unary_exprContext) ctx).start.getLine()
                        + ":" + ((RzParser.Unary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                throw new SemanticException(filePos + "Semantic Error: Unary operator cannot be apply to the '" + utype.toString() + "'");
            }
        }
        String filePos = "line " +  ((RzParser.Unary_exprContext) ctx).start.getLine()
                + ":" + ((RzParser.Unary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid unary expression");
    }
        ...
        if (ctx.getChild(0) instanceof RzParser.Postfix_exprContext) {
            ...
            if (identPrimary instanceof Variable) {
                ...
                if (typePrimary instanceof ArrayType
                    ...
                    String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                            + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                    throw new SemanticException(filePos + "Semantic Error: Invalid subscript '" +  ctx.getText() + "'");
                }
                ...
                if (typePrimary instanceof ClassType
                    ...
                    String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                            + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                    throw new SemanticException(filePos + "Semantic Error: Invalid class member '" + ctx.getText() + "'");
                }
                    ...
                    if (typePrimary instanceof StringType) {
                        if (ctx.getChild(1) instanceof RzParser.Build_in_FuntionContext) {
                            ...
                            if (((RzParser.Build_in_FuntionContext) ctx.getChild(1)).ident().getText().equals("substring")) {
                                if (ctx.getChild(1).getChildCount() > 4) {
                                    ...
                                    if (arg.assign_expr().size() == 2) {
                                        if ((getTypeofAssignExpr(arg.assign_expr(0), symt) instanceof IntType)
                                            ...
                                            String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                                                    + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                                            throw new SemanticException(filePos + "Semantic Error: unmatched argument" +
                                ...
                                String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                                        + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                                throw new SemanticException(filePos + "Semantic Error: number of parameters is not equal " +
                                ...
                                if (ctx.getChild(1).getChildCount() > 4) {
                                    ...
                                    if (arg.assign_expr().size() == 1) {
                                        if (getTypeofAssignExpr(arg.assign_expr(0), symt) instanceof IntType) {
                                            ...
                                            String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                                                    + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                                            throw new SemanticException(filePos + "Semantic Error: unmatched argument in build-in " +
            ...
            if (identPrimary instanceof FunctionType
                ...
                if (ctx.getChild(1).getChildCount() == 2 && ((FunctionType) identPrimary).getParamCount() == 0) {
                        ...
                        for (int i = 0; i < ((FunctionType) identPrimary).getParamCount(); ++i) {
                            if (ctx.getChild(1).getChild(1).getChild(2 * i) instanceof RzParser.Assign_exprContext) {
                                ...
                                if (!getTypeofAssignExpr(ctx.getChild(1).getChild(1).getChild(2 * i), symt).equals(
                                    ...
                                    String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                                            + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                                    throw new SemanticException(filePos + "Semantic Error: unmatched parameter '"
                    ...
                    String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                            + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                    throw new SemanticException(filePos + "Semantic Error: number of parameters is not equal to which " +
                            "in the function declaration '" + ctx.getChild(0).getText() + "'");
                }
                String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                        + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                throw new SemanticException(filePos + "Semantic Error: Invalid function call '" + ctx.getChild(0).getText() + "(...)'");
            }
            ...
            if (identPrimary instanceof ClassType
                ...
                String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                        + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                throw new SemanticException(filePos + "Semantic Error: Invalid member '" + memname + "'");
            }
...
//            }
                    ...
                    if (ctx.getChild(1).getChildCount() > 4) {
                        ...
                        if (arg.assign_expr().size() == 2) {
                            if ((getTypeofAssignExpr(arg.assign_expr(0), symt) instanceof IntType)
                                ...
                                String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                                        + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
                                throw new SemanticException(filePos + "Semantic Error: unmatched argument in " +
        ...
        String filePos = "line " +  ((RzParser.Postfix_exprContext) ctx).start.getLine()
                + ":" + ((RzParser.Postfix_exprContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid post_expression '" + ctx.getText() + "'");
    }
    ...
    public Type getTypeofPrimaryExpr(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        String filePos = "line " +  ((RzParser.Primary_exprContext) ctx).start.getLine()
                + ":" + ((RzParser.Primary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid primary expression '" + ctx.getText() + "'");
    }
    ...
    public Type getTypeofExpr(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        throw new SemanticException(filePos + "Semantic Error: Invalid expr '" + ctx.getText() + "'");
    }
    ...
    public Type getTypeofAssignExpr(ParseTree ctx, SymbolTable symt) {
        ...
        String filePos = "line " +  ((RzParser.Assign_exprContext) ctx).start.getLine()
                + ":" + ((RzParser.Assign_exprContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid assignment '" + ctx.getText() + "'");
    }
    ...
    public Identifier getIdentofUnaryExpr(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        if (ctx.getChild(1) instanceof RzParser.Unary_exprContext) {
            ...
            String filePos = "line " +  ((RzParser.Unary_exprContext) ctx).start.getLine()
                    + ":" + ((RzParser.Unary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: Unary operator cannot be apply to a " + utype.toString());
        }
        String filePos = "line " +  ((RzParser.Unary_exprContext) ctx).start.getLine()
                + ":" + ((RzParser.Unary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
        throw new SemanticException(filePos + "Semantic Error: Invalid unary expression");
            ...
            if (identPrimary instanceof Variable) {
                ...
                if (typePrimary instanceof ArrayType
                    ...
                    if (!getTypeofExpr(((RzParser.SubscriptContext) ctx.getChild(1)).expr(), symt).equals(new IntType())) {
                        throw new SemanticException(filePos + "Semantic Error: Invalid type in [ ] '" + ctx.getText() + "'");
                    }
                    int nowdim = (new ArrayType()).countdim(typePrimary.toString());
                    if (nowdim >= 2) {
                        ArrayType typehere = new ArrayType(typePrimary);
                ...
                if (typePrimary instanceof ClassType
                    ...
                    throw new SemanticException(filePos + "Semantic Error: Invalid member '" + ctx.getText() + "'");
                }
                if (typePrimary instanceof IntType
                        &&  (ctx.getChild(1) instanceof RzParser.PlusPlusContext
                        || ctx.getChild(1) instanceof RzParser.MinusMinusContext)) {
...
//                if (memtype != null) return new Variable(memtype);
    ...
    public Identifier getIdentofPrimary(ParseTree ctx, SymbolTable symt) throws SemanticException {
        ...
        if (ctx instanceof RzParser.Primary_identContext) {
            ...
            String filePos = "line " +  ((RzParser.Primary_exprContext) ctx).start.getLine()
                    + ":" + ((RzParser.Primary_exprContext)ctx).start.getCharPositionInLine() + " -> ";
            throw new SemanticException(filePos + "Semantic Error: Unknown identifier '" + ctx.getText() + "'");
        }
        if (ctx instanceof RzParser.Primary_const_intContext) {
            return new IntType();
        }
